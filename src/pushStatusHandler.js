...
export default function pushStatusHandler(config) {
  ...
  let setRunning = function(installations) {
    getLogger().verbose('sending push to %d installations', installations.length);
  ...
  let complete = function(results) {
    ...
    getLogger().verbose('sent push! %d success, %d failures', update.numSent, update.numFailed);
    lastPromise = lastPromise.then(() => {
  ...
  let fail = function(err) {
    ...
    getLogger().info('warning: error while sending push', err);
    lastPromise = lastPromise.then(() => {
